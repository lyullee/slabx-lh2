...
Sensor concentrations are not the quantity a risk assessment uses, and it
matters: on the FFI trials the model's agreement swings from FAC2 0.42 to 0.92
purely on an assumed humidity that was never measured.  **The LFL distance
does not move at all** across the same humidity sweep, because it is set by
where the profile crosses a threshold rather than by its absolute level.
...
LFL_HYDROGEN = 4.0
#: Smallest multiplier that makes all six FFI trials conservative is 1.23.
SAFETY_FACTOR = 1.25
