...
pressure.  ``f_deposit`` is the fraction of the remaining liquid that actually
reaches the ground.  It is deliberately an explicit input: this module does
not resolve jet impact, droplet trajectories, splashing, drainage, or
containment.  `E` is the evaporative mass flux, and it does not have to be
...
An historical consistency check against six reported pool radii gives a median
predicted/observed of 1.03 (1.11 including NASA, whose pool was confined):
...
This is **not an independent validation set**: it mixes campaigns, assumes
``f_deposit = 1``, and uses a fixed regression rate.  It is retained as an
audit record, not as evidence for an automatic pool-formation claim. NASA Test
6 is excluded: its pool was confined by the 9.1 m pond's 0.6 m clay walls, and
`max_radius` exists for that case.
...
__all__ = ["REGRESSION_RATE", "CRITICAL_HEAT_FLUX", "SUBSTRATES",
           ...
           "ground_limited_flux", "pool_radius", "ground_coupled_pool_source"]
...
def _deposited_liquid_rate(rate: float, vapour_frac: float,
                           deposition_fraction: float) -> float:
    """Liquid mass rate reaching the ground after flash and impact losses."""
    if rate < 0.0:
        raise ValueError(f"rate must be >= 0, got {rate!r}")
    if not 0.0 <= vapour_frac <= 1.0:
        raise ValueError(
            f"vapour_frac must be in [0, 1], got {vapour_frac!r}")
    if not 0.0 <= deposition_fraction <= 1.0:
        raise ValueError("deposition_fraction must be in [0, 1], "
                         f"got {deposition_fraction!r}")
    return rate * (1.0 - vapour_frac) * deposition_fraction


def equilibrium_radius(rate: float, vapour_frac: float, flux: float, *,
                       deposition_fraction: float = 1.0) -> float:
    """
    ...
    m_liquid = _deposited_liquid_rate(rate, vapour_frac, deposition_fraction)
    if m_liquid <= 0.0:
        return 0.0
...
def pool_radius(*, rate: float, tanker_barg: float, fluid: str,
    ...
    """
    ``{vapour_fraction, m_liquid, m_pool_in, flux, radius, area, confined}``.

    ``deposition_fraction`` is not fitted by this function.  A value of one
    means that every post-flash liquid kilogram reaches the pool; use an
    evidence-qualified range when impact or drainage is unresolved.
    ...
    m_liquid = rate * (1.0 - x)
    m_pool_in = _deposited_liquid_rate(rate, x, deposition_fraction)
    ...
    return {"vapour_fraction": x, "m_liquid": m_liquid,
            "deposition_fraction": deposition_fraction, "m_pool_in": m_pool_in,
            "flux": flux, "radius": r, "area": math.pi * r * r,
            "confined": confined}


def ground_coupled_pool_source(*, rate: float, tanker_barg: float,
                               fluid: str, elapsed: float,
                               substrate_name: str = "concrete_cryogenic",
                               deposition_fraction: float = 1.0,
                               max_radius: float | None = None,
                               T_ambient: float = 282.0,
                               p_ambient: float = 101325.0) -> dict[str, Any]:
    """Return one quasi-steady LH2 pool source term at an elapsed time.

    The returned area is the equilibrium area for the stated time-dependent
    ground heat flux.  It is *not* a transient impact/pool-spreading solver.
    Call this function at declared time windows to form a transparent source
    history for a dispersion calculation.  If a containment radius limits the
    equilibrium area, ``liquid_accumulation_rate`` is positive and the output
    must not be treated as a steady pool without a separate inventory model.
    """
    if rate <= 0.0:
        raise ValueError(f"rate must be > 0, got {rate!r}")
    if elapsed <= 0.0:
    ...
    p0 = tanker_barg * 1e5 + p_ambient
    if tanker_barg < 0.0:
        raise ValueError(f"tanker_barg must be >= 0, got {tanker_barg!r}")
    try:
        T_pool = float(CP.PropsSI("T", "P", p_ambient, "Q", 0, fluid))
        dh_vap = float(CP.PropsSI("H", "P", p_ambient, "Q", 1, fluid)
                       - CP.PropsSI("H", "P", p_ambient, "Q", 0, fluid))
        # Check the same physical domain as vapour_fraction before reporting
        # a source term whose upstream state cannot be a saturated liquid.
        if p0 >= CP.PropsSI("Pcrit", fluid):
            raise ValueError("stagnation pressure is at or above the critical "
                             "pressure; no saturated-liquid flash is defined")
    except ValueError:
        raise
    except Exception as exc:
        raise ValueError(f"unable to obtain saturation properties for {fluid!r}") from exc

    x = vapour_fraction(fluid, tanker_barg, p_ambient=p_ambient)
    flux = ground_limited_flux(substrate(substrate_name), dh_vap, T_ambient,
                                T_pool, elapsed)
    m_pool_in = _deposited_liquid_rate(rate, x, deposition_fraction)
    r_unconfined = equilibrium_radius(rate, x, flux,
                                      deposition_fraction=deposition_fraction)
    r = r_unconfined if max_radius is None else min(r_unconfined, max_radius)
    area = math.pi * r * r
    m_evap = area * flux
    confined = max_radius is not None and r_unconfined > max_radius
    return {
        "elapsed_s": elapsed,
        "vapour_fraction": x,
        "deposition_fraction": deposition_fraction,
        "m_pool_in_kgs": m_pool_in,
        "pool_temperature_K": T_pool,
        "latent_heat_Jkg": dh_vap,
        "evaporative_flux_kgm2s": flux,
        "unconfined_radius_m": r_unconfined,
        "radius_m": r,
        "area_m2": area,
        "evaporation_rate_kgs": m_evap,
        "liquid_accumulation_rate_kgs": max(m_pool_in - m_evap, 0.0),
        "confined": confined,
        "source_status": ("quasi_steady_equilibrium" if not confined
                          else "confined_pool_requires_inventory_model"),
    }
