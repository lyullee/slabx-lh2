...
2.674 q^0.135. An earlier round figure of 2.5 q^0.15 appears in document 22
and is superseded -- the two agree to 3 % over 0.1 to 30 kg/s, but the fitted
values are what `critical_wind` uses.

**Wind dominates.** A hundredfold change in rate moves the critical wind by a
factor of two.  Stability moves it by three: 2.1 m/s at B, 3.5 at D, 6.7 at F
...
CRITICAL_WIND_FIT: dict[str, tuple[float, float]] = {
    "A": (1.878, 0.118), "B": (1.968, 0.121), "C": (2.147, 0.124),
    "D": (2.674, 0.135), "E": (3.839, 0.129), "F": (4.848, 0.119),
}
...
def applicability(traj, *, marginal: float = 0.5,
    ...
    """
    Runtime status for a completed run: may this answer be used?

    ``{status, premise_ratio, angle_deg, x_at_max, reason, fallback_required}``
    with `status` one of ``VALID``, ``MARGINAL``, ``OUT_OF_SCOPE``.

    Written for a monitoring loop, where the question is not "what is the
    distance" but "am I entitled to report one". ``OUT_OF_SCOPE`` means the
    marching variable has stopped being a coordinate along the cloud and the
    number must not be shown, factored or otherwise.

    The band between `marginal` and `threshold` has no separate evidence
    behind it; it exists so that a system approaching the boundary can warn
    before crossing it.
    """
