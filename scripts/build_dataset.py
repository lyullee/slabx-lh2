...
RECORDS: list[tuple] = [
    ...
    # ------------------------------------------------------- negative control
    ("negctl.lng_width_gate.max_change", 0.0, "%", "A", "model",
     "ten LNG pool trials, identical clamped CoolProp water on both arms",
     "54",
     "**true negative control.** The hydrogen-only plume-width coupling is "
     "bit-inactive for LNG in every trial; the water treatment is held fixed"),
    ("lfl.unchanged_by_upstream_fix", True, "-", "A", "model",
     ...
     "not depend on which"),
    ("crossfluid.lng_burro8.water_treatment_change", 13.54, "%", "A",
     "model", "Burro 8, LFL distance, deliberately clamped versus corrected "
     "CoolProp water backend", "54",
     "**cross-fluid ablation, not a current software defect and not a negative "
     "control.** It shows that sub-triple-point water treatment can matter "
     "outside hydrogen; slabx 1.0.6 already carries the correction"),
    ...
    ("negctl.lng_pool.max_change.withdrawn_0_37", 0.37, "%", "-", "withdrawn",
     "hand calculation during the session", "26",
     "superseded first by 0.25 %, then withdrawn with the mixed-control design"),
    ("negctl.lng_pool.max_change.withdrawn_0_25", 0.25, "%", "-",
     "withdrawn", "legacy water wrapper comparison", "42",
     "**withdrawn.** It mixed the water treatment with the width gate and "
     "therefore was not a negative control"),
    ("negctl.burro8.change.withdrawn_0_27", 0.27, "%", "-", "withdrawn",
     "Burro 8, mixed module comparison", "24",
     "**withdrawn.** Replaced by a bit-identical width-gate negative control "
     "and a separate 13.54 % cross-fluid water ablation"),
    ...
    ("ucrit.D.a", 2.611924, "-", "A", "model",
     "bisected on max w_c/u with the width-coupled 0.1.3 configuration; fitted log-log",
     "30", ""),
    ("ucrit.D.b", 0.133865, "-", "A", "model", "as above", "30", ""),
    ...
    ("ucrit.withdrawn", "2.5 q^0.15", "-", "-", "withdrawn",
     ...
     "historical approximation; not what critical_wind uses"),
    ...
    ("runtime.warm_e2e_p95_ffi4", 65.648925, "ms", "-", "model",
     "i9-12900K, Python 3.12.14, slabx 1.0.6, slabx-lh2 0.1.3, n = 100", "60",
     "p99/p50 = 1.080, no failures; 380-row raw record and hashes archived"),
    ("runtime.warm_e2e_p95_out_of_scope_ms", 149.077435, "ms", "-", "model",
     "NASA test 6, same 0.1.3 run, n = 100", "60",
     "the slowest of the three single cases: **refusing costs a full "
     "integration**"),
    ("runtime.batch35_sequential_p95", 4196.8819, "ms", "-", "model",
     "35 scenarios sequentially, same 0.1.3 run, n = 30", "60",
     "p99/p50 = 1.015. FFI 6, PRESLHY E3.5 17, NASA 4, Zhang 8"),
    ("runtime.batch35_parallel.not_quotable", "3.7 to 3.9", "-", "-",
     "model", "35 scenarios on 6 and on 4 processes, n = 30 each", "53",
     "**do not quote.** p99/p50 is 3.67 on six workers and 3.88 on four -- "
     "reducing the worker count makes it worse, so this is not a tuning "
     "problem. The parallel run also degrades the sequential measurement in "
     "the same process, from p99/p50 1.06 to 1.36"),
    ("runtime.cold_over_warm", 23.6009, "-", "A", "diagnostic",
     "cold start p50 against warm p50, same 0.1.3 run", "60",
     "**a per-request subprocess is not viable.** Use a pre-warmed worker"),
    ("runtime.out_of_scope_is_the_slowest", True, "-", "A", "diagnostic",
     "NASA Test 6 against the two FFI cases, same 0.1.3 run", "60",
     "**refusing costs a full integration.** p95 149.077 ms against 65.649 "
     "and 117.070 ms"),

    ("runtime.workers_must_be_processes", True, "-", "-", "diagnostic",
     "the module-global patch plus the cold-start cost", "31, 37",
     "threads raise ConcurrentPatchError, and a fresh process costs about "
     "thirty warm calls, so the pool has to be pre-warmed processes"),
    ...
    ("regulatory.lng_endpoint_concentration", 2.5, "vol %", "-",
     ...
     "0.03 m"),
    ("lfl.grid_independence.withdrawn", 0.001, "fraction", "-", "withdrawn",
     "a single trial, taken as representative", "50",
     "**withdrawn.** Re-measured on all six over x_max 120 to 1200 m, the "
     "spread is 0.07 to 1.65 %"),
    ("lfl.grid_independence", 0.0165, "fraction", "A", "model",
     "all six FFI trials, x_max 120 to 1200 m", "50",
     "worst of six: 0.07 % on test 4, **1.65 % on test 6**. The lofting "
     "trial is the sensitive one, because where the integration stops "
     "changes how much of the rise is resolved. `n_puff_steps` has no effect "
     "in plume mode. After interpolation; before it, four trials returned an "
     "identical 40.75 m because the geometric grid is shared"),
    ...
    ("entrainment.excess_vs_free_jet.withdrawn", "4.4 to 5.4", "-", "-",
     "withdrawn", "an arithmetic error", "47",
     "**withdrawn.** The Ricou-Spalding comparison used a mass-weighted "
     "source density of 40.2 kg/m3 where the model uses 2.686. Corrected, "
     "the ratio is 1.17 to 1.48"),
    ("entrainment.excess_at_30m.withdrawn", 2.6, "-", "-", "withdrawn",
     "R_flux[0] taken as the release rate", "52",
     "**withdrawn**, and the 'three symptoms, one number' claim with it. "
     "R_flux[0] is half the release rate and R_flux is a half flux; the "
     "total at 30 m is 254.63 kg/s"),
    ("entrainment.excess.withdrawn_1_72", 1.72, "-", "-", "withdrawn",
     "R_flux read as the total", "52",
     "**withdrawn.** Half the true value; see rflux.is_a_half_flux"),
    ("displacement.withdrawn_1_65", 1.65, "-", "-", "withdrawn",
     "difference of two ratios, one of them from a half flux", "52",
     "**withdrawn.** Use the qualitative statement instead"),
    ("entrainment.withdrawn_span", "0.8 to 5.9", "-", "-", "withdrawn",
     "five assumptions treated as independent", "52",
     "**withdrawn.** The advection speed cancels, the mixture density never "
     "entered, and peak-to-bulk and the shape factor are the same knob. Two "
     "explicit scenarios replace it"),
    ("entrainment.conditional_ratio.t6_half_peak", 2.479, "-", "C",
     "comparison", "FFI test 6, 2*R_flux against the arc, bulk = 0.5 peak",
     "54", "**illustrative conditional scenario, not a verified Gaussian "
     "profile and not a bound.** Test 4 gives 2.076 under the same assumption"),
    ("entrainment.conditional_ratio.t6_peak_equals_bulk", 5.560, "-", "C",
     "comparison", "FFI test 6, bulk = peak", "54",
     "**extreme conditional scenario, not a lower bound.** It minimizes the "
     "inferred flux and therefore maximizes the model/inferred ratio; Test 4 "
     "gives 4.547"),
    ("humidity.bracket_count_depends_on_it", "5 of 6 at RH >= 75 %, 3 of 6 "
     "at RH <= 50 %", "-", "B", "diagnostic",
     "six FFI trials at RH 0, 25, 50, 75, 90, 100 %", "53",
     "**quote the assumed humidity wherever the count is quoted.** Tests 1 "
     "and 6 cross from inside the bracket to conservative below 50 %, and "
     "their LFL distances move by 54 and 57 % across the range"),
    ("humidity.horizontal_underprediction_at_every_rh", "0.29 to 0.40", "-", "B",
     "diagnostic", "the two horizontal trials over the full RH range", "53",
     "both horizontal trials remain underpredicted throughout RH 0--100 %. "
     "This does **not** imply a strict orientation split: at RH 100 % one "
     "downward trial also remains underpredicted"),
    ("humidity.strict_orientation_split_at_every_rh", False, "-", "B",
     "diagnostic", "six FFI trials over RH 0--100 %", "54",
     "the stronger orientation-separation claim is false at RH 100 %"),
    ("sensors.fixed_x_cross_section_identifiable", False, "-", "A",
     "diagnostic", "15 polar-arc sensors at nominal radius 30 m", "54",
     "the arc is not a fixed downwind plane after wind rotation: x spans "
     "18.88--29.84 m in Test 4 and 10.26--29.97 m in Test 6. Temporal means "
     "also cannot be divided by temporal maxima to infer a spatial "
     "peak-to-bulk factor"),
    ("sensors.lateral_mean_over_peak.withdrawn", "0.34 to 0.41", "-", "-",
     "withdrawn", "polar-arc time means divided by temporal maxima", "53",
     "**withdrawn.** The calculation treated a polar arc as a fixed-x "
     "cross-section and mixed temporal means with maxima"),
    ("sensors.vertical_mean_not_identifiable", True, "-", "A", "diagnostic",
     "sensors reach 1.8 m; the model cloud is 5.7 to 7.7 m deep", "53",
     "**the cross-sectional mean the mass balance needs cannot be measured.** "
     "2.479 and 5.560 are diagnostic scenarios under stated assumptions, not "
     "measurements"),
    ("model.total_flux_t6_30m", 254.630, "kg/s", "A", "model",
     "2 * R_flux from the trajectory", "52",
     "**no assumption on the model side.** Report both scenarios and do not "
     "average them; there is no distribution over the peak-to-bulk "
     "conversion"),
    ("rflux.is_a_half_flux", True, "-", "A", "measurement",
     "2 * R_flux[0] against the release rate, six FFI trials", "52",
     "**exact to twelve figures.** SLAB integrates one side of a symmetric "
     "cloud. Reading R_flux as the total halves every entrainment ratio; "
     "pinned by TestRFluxIsAHalfFlux"),
    ("displacement.cloud_above_sensors", True, "-", "B", "diagnostic",
     "FFI test 6 at 30 m", "52",
     "the model places the centreline at 2.46 m and the sensors reach "
     "1.8 m. **Qualitative only** -- an earlier numerical decomposition was "
     "withdrawn because it was taken from a half flux"),
    ("entrainment.free_jet_also_excessive", 2.2, "-", "A", "comparison",
     "Ricou-Spalding at 30 m against the measurement", "48",
     "**the free-jet law over-predicts too.** A cold jet in a co-flowing "
     "wind entrains about half of what a free jet in still air does, and "
     "neither closure represents that"),
    ("depth.not_independently_adjustable", True, "-", "A", "diagnostic",
     "thermal-convection ablation", "48",
     "removing the thermal term drops u* by 14 % and the depth by 4 %: R "
     "falls, B falls with it, and h barely moves"),
    ("ustar.inflation_at_30m", 2.08, "-", "A", "diagnostic",
     "FFI test 6, in-cloud against ambient friction velocity", "48",
     "split about evenly between retained source shear in the ground term "
     "and thermal convection, which itself scales as h^(2/3) and so feeds "
     "back. **Both are defensible physics**; the sum is too much here"),
    ("depth.vs_passive_limit", 3.0, "-", "A", "diagnostic",
     "FFI test 6 at 30 m against Pasquill-Gifford D", "47",
     "the modelled cloud is 8.1 m wide and 7.8 m deep; the passive layer is "
     "7.2 m and 2.6 m. **The width is right to 10 % and the depth is three "
     "times too large**, which is the whole factor of 2.6"),
    ("depth_cap.rejected", "three registrations", "-", "A", "diagnostic",
     "PREREG_lh2_depth_cap, _depth_plume, _depth_virtual", "prereg",
     "**all three rejected on the dense-gas controls**: 36 %, then 1.33 % "
     "with the right stability curves, then 17 % with the measured k. "
     "Lowering the ceiling made it worse, which is the signature of a "
     "constraint removing real physics"),
    ("slabx.passive_depth_ratio", 1.09, "-", "A", "measurement",
     "Prairie Grass, depth against Briggs sigma_z", "prereg",
     "**SLAB's own passive asymptote**: 1.87 at 10 m, 1.20 at 100 m, 1.09 "
     "from 300 m out. Nobody had written this down; the textbook top-hat "
     "factor of 1.5 is above anything the model produces passively"),
    ("premise.depth_is_not_bounded_by_passive", True, "-", "A", "diagnostic",
     "three rejected registrations", "prereg",
     "**a cloud can be thicker than a passively dispersing layer.** Near the "
     "source it has gravity-driven spreading and shear of its own, so "
     "'turbulence sets the maximum depth' is false and a ceiling is the "
     "wrong shape of correction"),
    ("control.passed_while_inert", 3, "occurrences", "A", "diagnostic",
     "bug 8, and PREREG_lh2_depth_cap NC1/NC2", "prereg",
     "**a control that passes while the code under test does nothing has "
     "tested nothing.** vertical_drag patched only the submodule; depth_cap's "
     "frame walk was broken. Both reported clean nulls. "
     "`depth_cap.fired_count` now makes it checkable"),
    ("entrainment.vs_free_jet", "1.17 to 1.48", "-", "A", "diagnostic",
     "Ricou-Spalding law, 2 to 31 m, FFI test 6, correct source density",
     "47", "inside the scatter of the coefficient; **the entrainment is not "
     "the defect**"),
    ("ffi.measurements_are_mutually_inconsistent", True, "-", "B",
     "diagnostic", "FFI test 6, 30 m arc", "47",
     "the 21 vol% maximum implies -56 C and the coldest reading is -25.7 C. "
     "**Maximum concentration and minimum temperature are at different "
     "sensors and different times and cannot be combined**"),
    ("ffi.temperature_sensors_unused", 616, "readings", "-", "measurement",
     "data/lh2_ffi_sensors.csv, kind = temperature", "47",
     "a second validation target that this work did not use until the "
     "entrainment question arose; it measures entrainment more directly "
     "than concentration does"),
    ("flacs.test6_30m", "21 measured, 22-23 FLACS, 6.6 here", "vol %", "C",
     "comparison", "Hansen & Hansen, PSEP 174 (2023) 376-390, table 3", "45",
     "**FLACS predicts the near field far better.** It uses a pseudo-source "
     "-- 37 % H2 and 63 % air by mass at 25 K and 70 m/s, pseudo-diameter "
     "0.11 m against the real 25.4 mm nozzle -- and the 70 m/s is selected, "
     "not derived"),
    ("flacs.test5_30m", "7.6 measured, ~7 FLACS, 6.95 here", "vol %", "C",
     "comparison", "as above", "45", "all three agree at 30 m on test 5"),
    ("flacs.test5_100m", "1.5 measured, 2.0 FLACS, 0.11 here", "vol %", "C",
     "comparison", "as above", "45",
     "**the worst cell.** The downward release is substituted by a "
     "horizontal jet here (docs/18) and that holds at 30 m but not at 100"),
    ("flacs.liftoff_agrees", True, "-", "B", "comparison",
     "tests 5 and 6", "45",
     "all three place test 6 as lifted and test 5 as grounded"),
    ("flacs.50m_missed_by_all", True, "-", "C", "comparison",
     "test 6, 50 m arc", "45",
     "measured 2 %, FLACS 8 %, this work 0.63 %. Hansen & Hansen attribute "
     "it to a diagonal wind pushing the plume off the centreline -- a "
     "property of the measurement, not of any model"),
    ("humidity.hansen_used_90pct", 90.0, "%", "-", "measurement",
     "Hansen & Hansen 2023, table 2", "45",
     "**FFI never reported it and this work assumes 75 %.** The difference "
     "alone moves the 30 m concentration by 16 %, which is why the distance "
     "to the flammable limit is the endpoint here"),
    ("crossmodel.test4_liftoff_agrees", True, "-", "B", "comparison",
     "Mack & Boot ICHS 2023 against scripts/cross_model.py", "44",
     "EFFECTS report no lift-off at any wind speed; Briggs L_p here is 3.8, "
     "grounded"),
    ("crossmodel.test6_liftoff_agrees", True, "-", "B", "comparison",
     "as above", "44",
     "EFFECTS report the 100 m sensor does not detect, a lifted plume; L_p "
     "here is 28.9, the only one of the six FFI trials above threshold"),
    ("crossmodel.near_field_ratio", "0.37 to 0.39", "-", "C", "comparison",
     "30 m arc maxima, tests 4 and 6", "44",
     "**Mack & Boot report the same signature on the same two trials** -- "
     "'maximum values close to release are underpredicted' -- and attribute "
     "it to the integral and RANS classes rather than to EFFECTS"),
    ("crossmodel.far_field_ratio", "0.80 to 1.03", "-", "C", "comparison",
     "50 and 100 m arc maxima", "44", ""),
    ("crossmodel.MG", 1.57, "-", "C", "comparison",
     "six arc maxima, tests 4 and 6, RH 75 % assumed", "44",
     "MG above 1 is under-prediction. **Computed on an assumed humidity** "
     "that FFI never reported; reported for comparison with the literature, "
     "not as a basis for any claim"),
