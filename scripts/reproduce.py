...
from slabx_lh2.water_ice import (already_corrected, clamped, p_sublimation,
                                 with_sublimation)
...
def section_dense():
    ...
    a, b = out["clamped reference"], out["corrected"]
    print(f"  observed 455 m; deliberate-ablation change "
          f"{abs(b - a) / a * 100:.2f} %")
...
def section_pool():
    ...
    r = sorted(ratios)
    med = (r[len(r) // 2] if len(r) % 2
           else 0.5 * (r[len(r) // 2 - 1] + r[len(r) // 2]))
    print(f"  median {med:.2f}   (n = {len(r)}; results.json uses the same "
          f"definition)")
...
def collect() -> dict:
    ...
    src = dict(rate=116.93, area=116.93 / (116.93 / 657.0), duration=107.0)
    burro = {}
    for label, water in (("clamped_reference", clamped(coolprop_water())),
                         ("corrected", with_sublimation(coolprop_water()))):
        with plume_width_coupling(False):
            traj, _ = run_dispersion(EvaporatingPool(substance=LNG, **src),
                                     atm, LegacyThermo(LNG), water,
    ...
    burro["change_pct"] = abs(
        burro["corrected"] - burro["clamped_reference"]
    ) / burro["clamped_reference"] * 100.0
    burro["comparison_type"] = "deliberate clamped-versus-corrected ablation"
    burro["note"] = ("cross-fluid evidence that the sub-triple-point water "
                     "treatment is not hydrogen-specific; this is not a "
                     "negative control and not the installed slabx defect")
    out["cross_fluid_water_effect_burro8"] = burro
    ...
    # Keep the true negative control (the H2-only width coupling is gated off
    # for LNG) separate from the cross-fluid water ablation.  Combining the
    # two changes used to make a non-zero physical effect look like a failed
    # negative control.
    lng_negative = {}
    lng_cross_fluid = {}
    E = 116.93 / 657.0
    for name, st, u, zr, z0, q, dur, tav, obs in LNG_TRIALS:
        negative = {}
        for lbl, cpl in (("width_off", False), ("width_on", True)):
            ...
            with plume_width_coupling(cpl):
                traj, _ = run_dispersion(
                    EvaporatingPool(substance=LNG, rate=q, area=q/E,
                                    duration=float(dur)),
                    a, LegacyThermo(LNG), clamped(coolprop_water()),
                    x_max=1400.0, n_puff_steps=40)
            f = concentration_field(traj, a, z=1.0, t_avg=float(tav),
                                    t_release=float(dur))
            negative[lbl] = f.distance_to(0.05)
        negative["observed_m"] = float(obs)
        negative["change_pct"] = abs(
            negative["width_on"] - negative["width_off"]
        ) / negative["width_off"] * 100.0
        negative["pass"] = bool(negative["change_pct"] < 1e-9)
        lng_negative[name] = negative

        effect = {}
        for lbl, water in (
                ("clamped_reference", clamped(coolprop_water())),
                ("corrected", with_sublimation(coolprop_water()))):
            a = Atmosphere(u_ref=u, z_ref=zr, T=290.0, rh=50.0, z0=z0,
                           stability=st)
            with plume_width_coupling(False):
                traj, _ = run_dispersion(
                    EvaporatingPool(substance=LNG, rate=q, area=q/E,
                                    duration=float(dur)),
                    a, LegacyThermo(LNG), water, x_max=1400.0,
                    n_puff_steps=40)
            f = concentration_field(traj, a, z=1.0, t_avg=float(tav),
                                    t_release=float(dur))
            effect[lbl] = f.distance_to(0.05)
        effect["observed_m"] = float(obs)
        effect["change_pct"] = abs(
            effect["corrected"] - effect["clamped_reference"]
        ) / effect["clamped_reference"] * 100.0
        lng_cross_fluid[name] = effect

    lng_negative["max_change_pct"] = max(
        v["change_pct"] for v in lng_negative.values() if isinstance(v, dict))
    lng_negative["all_pass"] = all(
        v["pass"] for v in lng_negative.values() if isinstance(v, dict))
    lng_negative["note"] = ("true negative control: the H2-only plume-width "
                            "coupling must be bit-inactive for LNG")
    out["negative_control_width_gate_lng"] = lng_negative

    lng_cross_fluid["max_change_pct"] = max(
        v["change_pct"] for v in lng_cross_fluid.values()
        if isinstance(v, dict))
    lng_cross_fluid["upstream_already_corrected"] = bool(
        already_corrected(coolprop_water()))
    lng_cross_fluid["comparison_type"] = (
        "deliberate clamped-versus-corrected ablation")
    lng_cross_fluid["note"] = (
        "cross-fluid evidence that sub-triple-point water treatment matters "
        "outside hydrogen; the clamped arm deliberately reconstructs the "
        "pre-correction behaviour and is not the installed slabx backend")
    out["cross_fluid_water_effect_lng"] = lng_cross_fluid
    ...
    out["humidity_sensitivity"] = _humidity_summary()

    # --- definitions a reader needs before quoting anything ---------------
    out["definitions"] = {
        "R_flux": {
            "is": "half-plume mass flux",
            "total_is": "2 * R_flux",
            "check": "2 * R_flux[0] equals the release rate to twelve "
                     "figures on every FFI trial; pinned by "
                     "TestRFluxIsAHalfFlux",
            "why_it_matters": "reading it as the total halves every "
                              "entrainment ratio derived from it, which "
                              "happened once and is recorded in docs/52"},
        "entrainment_diagnostic": {
            "model_side": "2 * R_flux at the arc, from the trajectory; **no "
                          "assumption**",
            "measurement_side": "not observed; converting the polar-arc "
                                "temporal maximum to a fixed-x "
                                "cross-sectional mean requires unsupported "
                                "spatial and temporal assumptions",
            "cancellations": "given a bulk concentration, advection speed, "
                             "cross-sectional area and mixture density cancel",
            "not_identifiable": "the five bearings are on a polar arc, not "
                                "one downwind plane, and the three heights "
                                "stop at 1.8 m; neither lateral nor vertical "
                                "cross-sectional mean is identified",
            "scenarios": "conditional diagnostics only; neither is measured "
                         "or a bound. See paper_results/"
                         "entrainment_uncertainty.csv"},
        "premise_ratio": {
            "is": "max(w_c / u) along the trajectory",
            "means": "the ratio of the cloud's rise velocity to its "
                     "advection; the x-marching formulation presumes it is "
                     "small"},
        "safety_factor": {
            "is": "a rounded engineering margin",
            "is_not": "an independently validated safety factor -- it was "
                      "derived and evaluated on the same six FFI trials"},
    }

    out["provenance"] = {
        "humidity": "The accessible public FFI report does not tabulate "
                    "relative humidity. Every FFI figure here assumes 75 %. "
                    "See humidity_sensitivity for the full 0-100 % sweep",
        "slabx_version_note": "slabx 1.0.5 corrected saturation pressure and "
                              "1.0.6 added fusion enthalpy. slabx-lh2 checks "
                              "the two behaviours independently so 1.0.4, "
                              "1.0.5 and 1.0.6 are not conflated",
        "runtime": "hardware-specific. The reference measurement is an "
                   "i9-12900K workstation; container and laptop figures are "
                   "withdrawn",
    }
    return out


def _slabx_version() -> str:
    """
    The installed slabx version.

    The package exposes no `__version__`, so this reads the distribution
    metadata. **The version matters**: 1.0.5 carries only the pressure change;
    1.0.6 also carries the fusion enthalpy.
    """
    try:
        from importlib.metadata import version
        return version("slabx")
...
def _humidity_summary():
    """
    What the assumed humidity costs, read from the sweep if it has been run.

    **The bracket count depends on it** -- five of six at RH >= 75 % and three
    of six at RH <= 50 % -- so a reader who quotes the count without the
    humidity is quoting half a result.
    """
    f = _Path(__file__).resolve().parent.parent / "paper_results" \
        / "humidity_sensitivity.json"
    if not f.exists():
        return {"note": "run scripts/humidity_sensitivity.py"}
    d = _json.loads(f.read_text(encoding="utf-8"))
    return {"rh_grid_pct": d["rh_grid_pct"],
            "baseline_rh_pct": d["baseline_rh_pct"],
            "bracket_counts": d["bracket_counts"],
            "trials_ever_outside": d["trials_ever_outside"],
            "horizontal_underprediction_at_every_rh":
                d["horizontal_underprediction_at_every_rh"],
            "orientation_separated_at_every_rh":
                d["orientation_separated_at_every_rh"],
            "horizontal_ratio_span": [
                min(v["horizontal_min"] for v in d["by_rh"].values()),
                max(v["horizontal_max"] for v in d["by_rh"].values())],
            "note": "both horizontal trials remain under-predicted, but "
                    "strict horizontal/downward separation fails at 100 % "
                    "RH and the bracket count also changes"}
