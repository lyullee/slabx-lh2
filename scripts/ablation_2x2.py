...
Writes `ablation_2x2.csv`, `negative_controls.csv`, and
`cross_fluid_water_effect.csv`.
...
The NASA columns are where the coupling acts. A passive release and the
non-lofted LNG width gate are negative controls. The LNG water comparison is
not: it is a cross-fluid transfer test for the cryogenic water correction and
is reported separately.

**Nothing is retuned after seeing this.**

A note on the "off" arm
-----------------------
`W0` uses a **deliberately clamped** water backend rather than an unwrapped
one. slabx 1.0.6 corrected the clamp upstream, so an unwrapped backend already
carries the correction; running the design that way makes every `W0` cell
equal its `W1` neighbour and the ablation reports a null result it never
tested. `slabx_lh2.water_ice.clamped` reproduces the pre-1.0.5 behaviour so
that the comparison means the same thing on any version.
"""
...
from slabx_lh2.water_ice import (already_corrected, clamped,      # noqa: E402
                                 with_sublimation)


def _water(ice: bool, legacy: bool = False):
    """
    The water backend for one arm of the 2x2.

    The "off" arm has to be **clamped**, not merely unwrapped: slabx 1.0.6
    corrected the clamp upstream, so an unwrapped backend already carries the
    correction and every cell of the design returns the same answer. The
    ablation then looks like a clean null result when in fact it never ran.
    """
    base = water_backend() if legacy else coolprop_water()
    return with_sublimation(base) if ice else clamped(base)
...
def run_ffi(trial, ice, coupling):
    ...
    traj = _traj(src, atm, CoolPropThermo(H2, fluid="Hydrogen"),
                 _water(ice), coupling)
...
def run_lng(spec, ice, coupling):
    ...
    src = EvaporatingPool(substance=LNG, rate=q, area=q / (116.93 / 657.0),
                          duration=float(dur))
    traj = _traj(src, atm, LegacyThermo(LNG), _water(ice),
                 coupling, x_max=1400.)
...
def run_passive(ice, coupling):
    ...
    traj = _traj(src, atm, LegacyThermo(SO2), _water(ice, legacy=True),
                 coupling, x_max=600.)
...
def main() -> int:
    ...
    # -- negative controls and cross-fluid transfer ------------------------
    print("  negative controls and cross-fluid water effect")
    nc = []
    cross_fluid = []
    for spec in LNG_TRIALS:
        # Width coupling is gated off on these non-lofted dense clouds. Keep
        # the same deliberately clamped water backend on both arms.
        base = run_lng(spec, False, False)
        var = run_lng(spec, False, True)
        nc.append(dict(dataset="LNG pool", case_id=spec[0],
                       module="plume_width", metric="LFL distance [m]",
        ...
        # This is deliberately *not* a negative control. It quantifies how
        # much the water defect can move another cryogenic material.
        corrected = run_lng(spec, True, False)
        cross_fluid.append(dict(
            dataset="LNG pool", case_id=spec[0],
            metric="LFL distance [m]", clamped_water=base,
            iapws_ice_water=corrected,
            relative_change_pct=abs(corrected - base) / base * 100,
            interpretation="cross-fluid effect; no pass/fail criterion"))
    a, b = run_passive(False, False), run_passive(True, True)
    nc.append(dict(dataset="Prairie Grass", case_id="SO2 passive",
                   module="water+plume_width", metric="z_c array",
                   baseline="", variant="",
    ...
    with (out / "negative_controls.csv").open("w", newline="",
                                              encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=["dataset", "case_id", "module",
        ...
        w.writeheader()
        w.writerows(nc)
    with (out / "cross_fluid_water_effect.csv").open(
            "w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=list(cross_fluid[0]))
        w.writeheader()
        w.writerows(cross_fluid)
    ...
    worst_nc = max(x["relative_change_pct"] for x in nc
                   if x["dataset"] == "LNG pool")
    worst_cross = max(x["relative_change_pct"] for x in cross_fluid)
    print(f"\nnegative controls: LNG width-gate worst change "
          f"{worst_nc:.3g} %, passive bit-identical "
          f"{bool(nc[-1]['bit_identical'])}")
    print(f"cross-fluid water effect: LNG maximum {worst_cross:.3f} %")
    print(f"wrote {out/'ablation_2x2.csv'}, negative_controls.csv, and "
          "cross_fluid_water_effect.csv")
    return 0
