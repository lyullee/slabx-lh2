...
`max w_c/u`, computed on that trial.
...
def build(cases: dict | None = None):
    ...
    ax.set_title("Input-space map and post-run kinematic routing outcomes",
                 fontsize=11, pad=10)
    ax.text(1.2, 0.030, "routed: computed $w_c/u > 1$", fontsize=9,
            color="#8d2420", ha="center", va="center")
    ax.text(0.35, 12.0, "not routed by this diagnostic: $w_c/u \\leq 1$", fontsize=8.5,
    ...
    handles += [
        Line2D([], [], ls="none", marker="o", ms=6.8, mfc="#c9302c",
               mec="#7d1d1a", label="routed by post-run diagnostic"),
...
def main() -> int:
    ...
    for name, trials in cases.items():
        for x, y, ratio, _kind in trials:
            computed_in = ratio <= 1.0
            screened_in = y >= critical_wind(x, "D")
            n_in += computed_in
            n_out += not computed_in
            agree += computed_in == screened_in
            disagree += computed_in != screened_in
    print(f"  computed diagnostic: {n_in} inside, {n_out} outside")
