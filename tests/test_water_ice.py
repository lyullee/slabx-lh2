...
from slabx_lh2.water_ice import (DH_FUSION, P_TRIPLE, T_TRIPLE, SublimationWater,
...
class TestWrapperContract:
    ...
    def test_wraps_the_legacy_backend_too(self):
        w = with_sublimation(water_backend())
        assert w.saturation_ratio(200.0) < w.saturation_ratio(240.0)
...
class TestUpstreamAdoption:
    ...
    def test_the_legacy_backend_is_left_alone(self):
        """
        It extrapolates Antoine rather than clamping, so it is detected as
        corrected -- but it is 2.35x the sublimation pressure at 200 K. The
        docstring says why leaving it alone is right; this pins the number.
        """
        from slabx_lh2.water_ice import already_corrected, p_sublimation
        legacy = water_backend()
        assert already_corrected(legacy)
