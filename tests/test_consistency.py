...
RETRACTED = {
    ...
    r"0\.37\s?%": "0.0 % for the width-gate negative control; the water "
        "effect is a separate cross-fluid ablation",
    r"2\.5\s?q\^0\.15": "CRITICAL_WIND_FIT; class D is 2.611924 q^0.133865",
    ...
    r"결과가 ±0\.1\s?%|0\.1% 이내": "0.07 to 1.65 % over all six trials",
    r"유입 \*\*2\.6배\*\*|한 숫자.*2\.6": "2.479 or 5.560 depending on the "
        "peak-to-bulk scenario; see docs/52",
    r"127\.3 kg/s": "254.630 -- R_flux is a half flux",
    r"\*\*1\.72배\*\*|연직 변위 \*\*1\.65": "withdrawn; see docs/52",
    r"0\.8 ~ 5\.9|0\.8 to 5\.9": "two explicit scenarios, not a span",
    r"\b4136\b": "3900 (results.json, water_saturation_error)",
    r"dense-gas set by at most 0\.25": "the correction cannot reach the "
        "published dense-gas validation; see docs/42",
...
def test_the_generated_numbers_still_match_the_headline_claims():
    ...
    assert d["negative_control_width_gate_lng"]["all_pass"]
    assert d["cross_fluid_water_effect_burro8"]["change_pct"] > 10.0
    ...
    # Current slabx reports the corrected curve directly.
...
PUBLIC_SCRIPTS = [
    ...
    ("cross_model.py", []),
    ("entrainment_uncertainty.py", []),
    ("ffi_summary_table.py", []),
    ("sensor_audit.py", []),
    ("humidity_sensitivity.py", []),
    ("fit_critical_wind.py", ["--check-existing"]),
    ("ablation_2x2.py", []),
...
def test_every_script_runs_without_the_measurements(script, args):
    ...
    **Two scripts did not, and both failed the same way** -- the code that
    ...
    env = dict(os.environ, SLABX_LH2_DATA="/nonexistent-on-purpose",
               MPLBACKEND="Agg", PYTHONPATH=str(ROOT))
