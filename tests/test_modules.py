...
from slabx_lh2.pool import (equilibrium_radius, flux_from_regression,
                           ground_coupled_pool_source, pool_radius,
                           vapour_fraction)                       # noqa: E402
...
class TestPoolRadius:
    ...
    def test_scales_as_sqrt_deposition_fraction(self):
        full = equilibrium_radius(1.0, 0.0, 0.07, deposition_fraction=1.0)
        quarter = equilibrium_radius(1.0, 0.0, 0.07,
                                     deposition_fraction=0.25)
        assert quarter / full == pytest.approx(0.5)

    def test_no_deposition_gives_no_pool(self):
        assert equilibrium_radius(1.0, 0.0, 0.07,
                                  deposition_fraction=0.0) == 0.0
    ...
    def test_historical_radius_consistency_within_a_factor_of_two(
            self, rate, barg, observed):
        """Regression guard only; this heterogeneous set is not validation."""
    ...
    def test_ground_coupled_source_balances_an_unconfined_pool(self):
        got = ground_coupled_pool_source(rate=0.1055, tanker_barg=1.0,
                                         fluid="Hydrogen", elapsed=300.0)
        assert got["source_status"] == "quasi_steady_equilibrium"
        assert got["evaporation_rate_kgs"] == pytest.approx(
            got["m_pool_in_kgs"])

    def test_ground_coupled_source_reports_confined_accumulation(self):
        got = ground_coupled_pool_source(rate=9.5, tanker_barg=0.1,
                                         fluid="Hydrogen", elapsed=300.0,
                                         max_radius=0.5)
        assert got["confined"]
        assert got["liquid_accumulation_rate_kgs"] > 0.0
        assert got["source_status"] == "confined_pool_requires_inventory_model"
...
class TestConcurrency:
    ...
    def test_the_same_thread_may_nest(self, module):
        """
        Nesting in one thread is allowed and unwinds to the original.

        This used to have no assertion at all -- it ran the nesting and
        proved nothing, which is the same shape of hole as a control that
        passes against inert code.
        """
        import importlib
        import slabx.core.plume as plume
        import slabx.submodels.entrainment as ent
        m = importlib.import_module(f"slabx_lh2.{module}")
        cm = getattr(m, "plume_width_coupling" if module == "plume_width"
                     else "vertical_drag")
        before = (plume.entrainment, plume.fluxes, ent.fluxes)
        with cm(True):
            outer = (plume.entrainment, plume.fluxes, ent.fluxes)
            assert outer != before, "the patch was never installed"
            with cm(False):
                assert (plume.entrainment, plume.fluxes,
                        ent.fluxes) == before, (
                    "the inner active=False block did not disable it")
            assert (plume.entrainment, plume.fluxes,
                    ent.fluxes) == outer, "the outer patch was not restored"
        assert (plume.entrainment, plume.fluxes, ent.fluxes) == before
...
# --------------------------------------------------------------------------
class TestTheAblationActuallySeparates:
    """
    The 2x2 must have four distinct arms, on every version of slabx.

    **It did not.** slabx 1.0.6 corrected the water clamp upstream, so an
    unwrapped backend already carries the correction and `W0` returned the
    same answer as `W1`. The design collapsed to a 1x2 and reported that the
    water correction changes nothing -- a null result it never tested. Caught
    by running the script on 1.0.6; pinned here so it cannot come back.
    """

    def test_clamped_and_corrected_differ_below_the_triple_point(self):
        from slabx.thermo.coolprop import coolprop_water
        from slabx_lh2.water_ice import clamped, with_sublimation
        base = coolprop_water()
        off, on = clamped(base), with_sublimation(base)
        assert off.saturation_ratio(200.0) / on.saturation_ratio(200.0) > 100

    def test_clamped_is_flat_below_the_triple_point(self):
        from slabx.thermo.coolprop import coolprop_water
        from slabx_lh2.water_ice import clamped
        off = clamped(coolprop_water())
        assert off.saturation_ratio(200.0) == pytest.approx(
            off.saturation_ratio(150.0), rel=1e-12)

    def test_clamped_is_untouched_above_the_triple_point(self):
        from slabx.thermo.coolprop import coolprop_water
        from slabx_lh2.water_ice import clamped
        base = coolprop_water()
        off = clamped(base)
        for T in (280.0, 290.0, 300.0):
            assert off.saturation_ratio(T) == base.saturation_ratio(T)

    def test_clamping_a_corrected_wrapper_unwraps_it_first(self):
        """Otherwise the clamp would sit on top of the correction."""
        from slabx.thermo.coolprop import coolprop_water
        from slabx_lh2.water_ice import clamped, with_sublimation
        base = coolprop_water()
        assert clamped(with_sublimation(base)).saturation_ratio(200.0) == \
            pytest.approx(clamped(base).saturation_ratio(200.0))

    def test_clamped_is_idempotent(self):
        from slabx.thermo.coolprop import coolprop_water
        from slabx_lh2.water_ice import clamped
        once = clamped(coolprop_water())
        assert clamped(once) is once

    @pytest.mark.slow
    def test_the_ablation_script_produces_four_distinct_arms(self):
        """
        Run it and check that at least one FFI trial separates on water and
        at least one NASA trial separates on the coupling. A run where every
        `W0` equals its `W1` is the collapse this class exists to catch.
        """
        import csv
        import pathlib
        import subprocess
        ...
        subprocess.run([sys.executable,
                        str(root / "scripts" / "ablation_2x2.py")],
                       cwd=root, check=True, capture_output=True, timeout=1800)
        rows = list(csv.DictReader(
            (root / "paper_results" / "ablation_2x2.csv").open(
                encoding="utf-8")))

        def cell(dataset, case, w, c, field):
            r = next(x for x in rows if x["dataset"] == dataset
                     and x["case_id"] == case
                     and x["water_correction"] == str(w)
                     and x["width_coupling"] == str(c))
            return float(r[field])

        water_moves = any(
            cell("FFI", f"test{t}", 0, 0, "lfl_m")
            != cell("FFI", f"test{t}", 1, 0, "lfl_m")
            for t in (1, 5, 6))
        assert water_moves, (
            "no FFI trial separates on the water correction; the ablation "
            "has collapsed. Is the 'off' arm actually clamped?")

        coupling_moves = any(
            cell("NASA", f"test{t}", 1, 0, "rise_exponent_fit")
            != cell("NASA", f"test{t}", 1, 1, "rise_exponent_fit")
            for t in (2, 4, 5, 6))
        assert coupling_moves, (
            "no NASA trial separates on the width coupling")


# --------------------------------------------------------------------------
class TestDepthCapIsRejected:
    """
    `depth_cap` is a rejected module, kept for the record.

    It targets a real deficit -- the cloud depth is three times the passive
    limit at 30 m -- and the near field does respond to it. But it holds `h`
    while `z_c` keeps growing, so the cloud leaves the sensor plane.

    These tests pin the mechanism and the failure so that neither is
    rediscovered.
    """

    def test_it_is_off_by_default(self):
        from slabx_lh2.depth_cap import is_enabled
        assert not is_enabled()

    def test_the_ceiling_is_the_published_correlation(self):
        """Briggs open country, unfitted, all six classes."""
        from slabx_lh2.depth_cap import sigma_z
        assert sigma_z(100.0, "D") == pytest.approx(5.60, abs=0.05)
        assert sigma_z(100.0, "A") == pytest.approx(20.0, abs=0.1)
        assert sigma_z(100.0, "F") == pytest.approx(1.55, abs=0.05)
        assert sigma_z(0.0) == 0.0

    def test_the_stability_class_is_used(self):
        """
        The first registration used class D for everything, and the LNG
        controls -- which contain class C and E releases -- moved by 36 %.
        Using the right curve for each cut that to 1.33 %.
        """
        from slabx_lh2.depth_cap import sigma_z
        assert sigma_z(100.0, "A") > sigma_z(100.0, "D") > sigma_z(100.0, "F")

    def test_the_factor_is_the_models_own_passive_limit(self):
        """
        1.09, measured from SLAB's passive asymptote, not the textbook 1.5.
        Asserting 1.5 put the ceiling above anything the model produces
        passively.
        """
        from slabx_lh2.depth_cap import TOP_HAT_FACTOR
        assert TOP_HAT_FACTOR == pytest.approx(1.09, abs=0.01)

    def test_the_cap_only_ever_reduces(self):
        from slabx_lh2.depth_cap import cap_depth, sigma_z, TOP_HAT_FACTOR
        ceiling = TOP_HAT_FACTOR * sigma_z(30.0)
        assert cap_depth(0.5 * ceiling, 30.0) == 0.5 * ceiling
        assert cap_depth(5.0 * ceiling, 30.0) == pytest.approx(ceiling)

    def test_the_virtual_origin_matches_the_source_depth(self):
        """
        The origin is placed so the ceiling equals the cloud's own depth
        there. Measuring from x = 0 instead crushes a pool source: sigma_z
        is 0.09 m at one metre.
        """
        from slabx_lh2.depth_cap import (TOP_HAT_FACTOR, sigma_z,
                                         virtual_origin)
        for h0 in (0.02, 0.5, 2.0, 5.0):
            for stab in ("C", "D", "E"):
                xv = virtual_origin(h0, stab)
                assert TOP_HAT_FACTOR * sigma_z(xv, stab) == pytest.approx(
                    h0, rel=1e-3)

    def test_it_fails_closed_without_a_position(self):
        """
        The position is read from the caller's frame. If slabx's internals
        change, the module must switch itself off rather than cap at the
        wrong distance.
        """
        from slabx_lh2.depth_cap import _caller_x
        assert _caller_x() is None                  # no integrator above us

    def test_the_context_manager_restores(self):
        import slabx.core.plume as plume
        from slabx_lh2.depth_cap import depth_cap
        before = plume.entrainment
        with depth_cap():
            assert plume.entrainment is not before
        assert plume.entrainment is before

    def test_it_restores_on_an_exception(self):
        import slabx.core.plume as plume
        from slabx_lh2.depth_cap import depth_cap
        before = plume.entrainment
        with pytest.raises(RuntimeError):
...
class TestAControlMustExerciseTheChange:
    """
    A negative control that passes while the code under test does nothing has
    tested nothing.

    **This has happened twice.** Bug 8: `vertical_drag` patched only the
    submodule, had no effect, raised no error, and its result was recorded as
    a clean null. `PREREG_lh2_depth_cap`: the frame walk was broken, the cap
    never fired, and NC1 and NC2 were recorded as passes.

    `depth_cap.fired_count` exists so the second cannot recur silently, and
    these tests pin the habit.
    """

    def test_the_counter_starts_at_zero_and_counts(self):
        import math
        ...
        from slabx_lh2 import depth_cap as DC
        ...
        h2 = Substance(name="H2", mw=0.002016, cp_vapour=14300.0,
                       ...
                       rho_liquid=70.8)
        atm = Atmosphere(u_ref=2.7, z_ref=10.0, T=277.0, rh=75.0, z0=0.01,
                         stability="D")
        src = HorizontalJet(substance=h2, rate=0.832,
                            area=math.pi * 0.0254 ** 2 / 4, duration=180.0,
                            liquid_fraction=0.559, height=0.5,
                            T_source=20.37)
        DC.reset_counts()
        assert sum(DC.fired_count().values()) == 0
        with DC.depth_cap():
            run_dispersion(src, atm, CoolPropThermo(h2, fluid="Hydrogen"),
                           with_sublimation(coolprop_water()), x_max=100.0,
                           n_puff_steps=20)
        c = DC.fired_count()
        assert c["calls"] > 0, "the patch was never called"
        assert c["ceiling"] > 0, (
            "the patch was called but never changed anything -- a control run "
            "in this state proves nothing")

    def test_it_counts_nothing_when_disabled(self):
        from slabx_lh2 import depth_cap as DC
        DC.reset_counts()
        with DC.depth_cap(False):
            pass
        assert sum(DC.fired_count().values()) == 0
...
class TestPatchesCompose:
    """
    `plume_width` and `depth_cap` both rebind `plume.entrainment`.

    **Installing one over the other used to disable it silently.** No error,
    no warning, and a run with both enabled was byte-identical to a run with
    only the second. On NASA test 6 the pair gave exactly the depth-cap
    answer and the width coupling was simply gone.

    **Third occurrence of this failure mode.** Bug 8: `vertical_drag` patched
    only the submodule. `PREREG_lh2_depth_cap`: the frame walk was broken and
    two negative controls passed against inert code. Each time, something did
    nothing and nothing said so.

    Both modules now chain onto whatever is already installed.
    """
    ...
    def _run(*mods):
        ...
        atm = Atmosphere(u_ref=2.2, z_ref=10.0, T=288.15, rh=29.0, z0=3e-3,
                         stability="D")
        src = EvaporatingPool(substance=h2, rate=11.53,
                              area=math.pi * 4.55 ** 2, duration=35.0)
        cm = {"pw": plume_width_coupling, "dc": depth_cap}
        with contextlib.ExitStack() as stack:
            for m in mods:
                stack.enter_context(cm[m]())
            traj, _ = run_dispersion(src, atm,
                                     CoolPropThermo(h2, fluid="Hydrogen"),
                                     with_sublimation(coolprop_water()),
                                     x_max=400.0, n_puff_steps=40)
        return float(np.asarray(traj.z_c)[-1])

    @pytest.mark.slow
    def test_the_order_does_not_matter(self):
        assert self._run("pw", "dc") == pytest.approx(self._run("dc", "pw"),
                                                      rel=1e-9)

    @pytest.mark.slow
    def test_neither_silently_disables_the_other(self):
        """The pair must differ from each alone, in the same direction."""
        base = self._run()
        pw, dc = self._run("pw"), self._run("dc")
        both = self._run("pw", "dc")
        assert both != pytest.approx(pw, rel=1e-6), (
            "the pair equals plume_width alone; depth_cap is inert")
        assert both != pytest.approx(dc, rel=1e-6), (
            "the pair equals depth_cap alone; plume_width is inert")
        # both suppress the rise, so the pair must suppress it further
        assert both < min(pw, dc) < base

    def test_the_chain_base_is_recorded(self):
        import slabx.core.plume as plume
        from slabx_lh2 import depth_cap as DC
        from slabx_lh2 import plume_width as PW
        original = plume.entrainment
        with PW.plume_width_coupling():
            inner = plume.entrainment
            with DC.depth_cap():
                assert DC._chain_base() is inner, (
                    "depth_cap did not chain onto plume_width")
            assert plume.entrainment is inner
        assert plume.entrainment is original

    def test_the_chain_unwinds_in_the_other_order(self):
        import slabx.core.plume as plume
        from slabx_lh2 import depth_cap as DC
        from slabx_lh2 import plume_width as PW
        original = plume.entrainment
        with DC.depth_cap():
            inner = plume.entrainment
            with PW.plume_width_coupling():
                assert PW._chain_base() is inner
            assert plume.entrainment is inner
        assert plume.entrainment is original
