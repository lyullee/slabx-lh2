...
slabx-lh2/                       ← GitHub · PyPI · Zenodo
  slabx_lh2/                     모델 코드 (A)
    ...
    trials/
      __init__.py                  조건 로더 (B)
      conditions.py  E3.5·NASA·Zhang·FFI 조건만
...
```
  ...
  docs/34_DATA_HANDOFF.md           관측값을 포함한 내부 자료 인계
  paper_results/                   투고 산출물
  scripts/same_test_flacs.py       관측값·공개 FLACS 결과 전사 포함
  scripts/edit_manuscript_*.py     로컬 투고문서 경로 포함
```
...
6. 공개본 트리 생성 → 무측정자료 검증 → ZIP/wheel 검사 → GitHub → PyPI → Zenodo
